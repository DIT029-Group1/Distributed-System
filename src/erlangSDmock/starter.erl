...
-export([start_sup_ch/1,start_root/0,read_messages/1,start_chsup_ch/2,spawn_actors/2,test/0,connect/0,send/2]).
...
start_root() ->
	case whereis(root_sup) of 
		undefined ->	{ok, Pid} = root_sup:start_link(),
						{ok, Pid};
		Pid -> 			io:format("Root supervisor is already running.~n"),
						{ok,Pid}
	end.	
...
%% Read through a list of messages that are sent from javascript to erlang example for testing.
...
server_read_messages(Server,[{From,Msg,To}|Xs]) -> Server ! {{From,Msg,To},self()},timer:sleep(700),
		receive 
				{{From,Msg,To},_} -> {From,Msg,To} % call function to send it back to java script
		end,
   server_read_messages(Server,Xs).		

%% Pid is the id of the child sd_simulator_OTP
spawn_actors(_,[]) -> no_more_actors;
spawn_actors(Pid,[_|Ys]) -> sup:start_actor(Pid),spawn_actors(Pid, Ys).
